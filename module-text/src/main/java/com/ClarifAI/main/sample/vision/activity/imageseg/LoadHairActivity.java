...
package com.ClarifAI.main.sample.vision.activity.imageseg;
...
import com.ClarifAI.main.sample.vision.activity.BaseActivity;
import com.ClarifAI.main.sample.vision.transactor.ImageTransactor;
import com.ClarifAI.main.sample.vision.transactor.StillImageSegmentationTransactor;
import com.ClarifAI.main.sample.vision.util.BitmapUtils;
import com.ClarifAI.main.sample.vision.views.color.ColorSelector;
import com.ClarifAI.main.sample.vision.views.overlay.GraphicOverlay;
import com.huawei.hms.mlsdk.imgseg.MLImageSegmentationSetting;
import com.huawei.mlkit.sample.R;

import java.util.Locale;

public class LoadHairActivity extends BaseActivity {
    private static final String TAG = "LoadPhotoActivity";

    private static final String[] CATEGORIES = {"背景", "头发"};

    private static final String[] CATEGORIES_EN = {"Background", "Hair"};
    ...
    private void initView() {
        ...
        if (this.isEngLanguage()) {
            categories = LoadHairActivity.CATEGORIES_EN;
        } else {
            categories = LoadHairActivity.CATEGORIES;
        }
        for (int i = 0; i < categories.length; i++) {
            ...
            textView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    MLImageSegmentationSetting setting = new MLImageSegmentationSetting.Factory().setAnalyzerType(MLImageSegmentationSetting.HAIR_SEG).create();
                    StillImageSegmentationTransactor transactor = new StillImageSegmentationTransactor(setting, LoadHairActivity.this.originBitmap, LoadHairActivity.this.preview, index);
                    transactor.setColor(LoadHairActivity.this.colorvalue);
                    LoadHairActivity.this.imageTransactor = transactor;
                    LoadHairActivity.this.imageTransactor.process(LoadHairActivity.this.originBitmap, LoadHairActivity.this.graphicOverlay);
                }
            });
        }
        ...
        this.colorSelector.setOnColorSelectorChangeListener(new ColorSelector.OnColorSelectorChangeListener() {
            ...
            public void onColorChanged(ColorSelector picker, int color) {
                LoadHairActivity.this.colorvalue = color;
            }
        ...
        this.colorSelector.post(new Runnable() {
            @Override
            public void run() {
                LoadHairActivity.this.selectLocalImage();
            }
        });
    }
    ...
    private void selectLocalImage() {
        ...
        this.startActivityForResult(intent, LoadHairActivity.REQUEST_SLECT_IMAGE);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == this.REQUEST_TAKE_PHOTOR && resultCode == RESULT_OK) {
            this.loadImage();
        } else if (requestCode == this.REQUEST_TAKE_PHOTOR && resultCode == RESULT_CANCELED) {
            this.finish();
        } else if (requestCode == this.REQUEST_SLECT_IMAGE && resultCode == RESULT_OK) {
            if (data != null) {
                this.mImageUri = data.getData();
            }
            this.loadImage();
        } else if (requestCode == this.REQUEST_SLECT_IMAGE && resultCode == RESULT_CANCELED) {
            this.finish();
        }
    }

    private void loadImage() {
        this.originBitmap = BitmapUtils.loadFromPath(LoadHairActivity.this, this.mImageUri, this.getMaxWidthOfImage(), this.getMaxHeightOfImage());
