...
package com.ClarifAI.main.sample.src.transactor;
...
import com.ClarifAI.main.sample.src.camera.FrameMetadata;
import com.ClarifAI.main.sample.src.util.BitmapUtils;
...
import com.ClarifAI.main.sample.src.util.NV21ToBitmapConverter;
        ...
        this.detectInImage(image).addOnSuccessListener(new OnSuccessListener<T>() {
            ...
            public void onSuccess(T results) {
                Runnable task = new Runnable() {
                    @Override
                    public void run() {
                        MLClassificationManager.getInstance().updateClassifications((List< MLImageClassification >) results);
                        BaseTransactor.this.processLatestImage(graphicOverlay);
                    }
                };

                if (Looper.myLooper() != Looper.getMainLooper()) {
                    // Currently not in UI thread, run task directly.
                    task.run();
                } else {
                    // In UI thread, submit to executorService.
